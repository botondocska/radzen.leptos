...
This was my way to begin learning Leptos... How about another webassembly framework in c next, jk. I enjoy working with both Blazor and Leptos, and I am hoping this UI library can become the home of many-many components, so everyone becomes more keen on making web based frontend a better place. - Rust can garantee so much instead of javascript, why waste all of that? I note that 99% of this is AI generated code, with little guide (once I implemented the button and some very basic elements, those became the example of "how?"). The optimisation phase will be the one where I will be rewriting all components to be much more idiomatic to how Leptos wants me to handle properties and logic.
...
- Since both frameworks are using webassembly, and both Rust and C# are typesafe languages, its easier to make the similar components with similar logic. And I mean it like this: AI will be much more restricted to solve things in ways that don't or shouldn't be functional, compared to script languages.   
...
- If the component has dependencies not implemented, we shouldnt start implementing given component at all - we have to map recursively what components use what - then start with the one that only has already implemented dependencies, or no dependencies at all.
    - We have to validate every component implemented, based on the C# version: no features can be missing at the end:  
        ...
        - Making notes on what feauters are different from just "translating the code":
            - The original Radzen library has a javascript library as well: I was not planning on adding that, instead use webassembly. 
