...
use crate::components::{
    ClassList, RadzenDropDown,
    base_component::{ComponentProps, use_radzen_base},
    dropdown_base::{DropDownItem, DropDownProps},
};
...
fn build_calendar_weeks(year: i32, month: u32) -> Vec<Vec<NaiveDate>> {
    ...
    (0..6)
        .map(|row| {
            (0..7)
                .map(|col| grid_start + Duration::days(row * 7 + col))
                .collect()
        })
        .collect()
}
...
pub fn RadzenDatePicker(
    ...
    let has_value = move || -> bool {
        if multiple {
            !multi_signal.get().is_empty()
        } else {
            value_signal.get().is_some()
        }
    };
    ...
    let input_section: AnyView = if inline {
        ...
        let tab_str = if disabled {
            "-1".to_string()
        } else {
            effective_tab.to_string()
        };
        ...
        let button_el: AnyView = if show_button {
            leptos::html::button()
                ...
                .attr(
                    "aria-expanded",
                    move || if open.get() { "true" } else { "false" },
                )
    ...
    Effect::new(move |_| {
        month_value_str.set(view_month.get().to_string());
    });
    Effect::new(move |_| {
        year_value_str.set(view_year.get().to_string());
    });
    ...
    let popup_child = move || -> AnyView {
        ...
        let popup_style = if inline {
            ...
            "position:absolute;z-index:var(--rz-popup-z-index,1000);left:0;top:100%;min-width:100%"
                .to_string()
        };
        ...
        let mut month_base = ComponentProps::default();
        {
            ...
            attrs.insert(
                "class".to_string(),
                "rz-calendar-month-dropdown".to_string(),
            );
        ...
        let rows: Vec<AnyView> = weeks
            ...
            .map(|week| {
                ...
                for day in week {
                    ...
                    let tab = if is_day_disabled || !is_cur_month {
                        "-1"
                    } else {
                        "0"
                    };
                    ...
                    cells.push(
                        leptos::html::td()
                            ...
                            .on(leptos::ev::click, move |_| {
                                if is_day_disabled || read_only || !is_cur_month {
                                    return;
                                }
                                if multiple {
                                    ...
                                    let new_date =
                                        if allow_clear && sel { None } else { Some(day) };
        ...
        let time_picker_child: AnyView = if show_time || time_only {
            ...
            let hour_el = leptos::html::div()
                ...
                .child(
                    leptos::html::button()
                        ...
                        .attr(
                            "class",
                            "rz-button rz-button-icon-only rz-variant-text rz-secondary",
                        )
                        .attr("tabindex", "-1")
                        .attr("disabled", disabled)
                        .on(leptos::ev::click, move |_| {
                            ...
                            let next_display = if display >= hour_max {
                                hour_min
                            } else {
                                display + hours_step
                            };
                        ...
                        .child(
                            leptos::html::span().attr("class", "notranslate rzi rzi-chevron-up"),
                        ),
                )
                .child(move || {
                    let v = display_hour();
                ...
                .child(
                    leptos::html::button()
                        ...
                        .on(leptos::ev::click, move |_| {
                            ...
                            let next_display = if display <= hour_min {
                                hour_max
                            } else {
                                display - hours_step
                            };
                        ...
                        .child(
                            leptos::html::span().attr("class", "notranslate rzi rzi-chevron-down"),
                        ),
                );
                ...
                .child(
                    leptos::html::button()
                        ...
                        .on(leptos::ev::click, move |_| {
                            ...
                            let m = pending_minute.get_untracked();
                            pending_minute.set(if m < minutes_step {
                                60 - minutes_step
                            } else {
                                m - minutes_step
                            });
                ...
                leptos::html::div()
                    ...
                    .child(
                        leptos::html::button()
                            ...
                            .child(
                                leptos::html::span()
                                    .attr("class", "notranslate rzi rzi-chevron-up"),
                            ),
                    )
                    .child(move || {
                    ...
                    .child(
                        leptos::html::button()
                            ...
                            .on(leptos::ev::click, move |_| {
                                ...
                                let s = pending_second.get_untracked();
                                pending_second.set(if s < seconds_step {
                                    60 - seconds_step
                                } else {
                                    s - seconds_step
                                });
                            ...
                            .child(
                                leptos::html::span()
                                    .attr("class", "notranslate rzi rzi-chevron-down"),
                            ),
                    )
                    .into_any()
            } else {
            ...
            if show_hour {
                time_children.push(hour_el.into_any());
            }
            if show_minutes {
                if show_hour {
                    time_children.push(sep());
                }
            ...
            if is12 {
                time_children.push(ampm_el);
            }
            if show_time_ok_button {
                time_children.push(ok_el);
            }
